...
type ApiResponse = {
  ...
  refreshedAt: string
  teams?: { ok: boolean; error?: string | null; dbName?: string; collection?: string }
}
...
function filterLabel(label: string, selected: string[], valueToLabel?: (v: string) => string) {
  if (selected.length === 0) return `${label}: All`
  if (selected.length === 1) return `${label}: ${valueToLabel ? valueToLabel(selected[0]) : selected[0]}`
...
export default function ActiveCandidatesPage() {
  ...
  const fetchCandidates = useCallback(
    async (mode: "initial" | "refresh" = "refresh") => {
      ...
      try {
        const res = await fetch(`/api/active-candidates?limit=1000`, { cache: "no-store", signal: ac.signal })
  ...
  const teamMembers = useMemo(() => {
    const members =
      teamFilter.length === 0
        ? MOCK_TEAMS.flatMap((t) => t.members.map((m) => ({ ...m, team_name: t.team_name })))
        : teamFilter.flatMap((t) => getMockMembersForTeam(t).map((m) => ({ ...m, team_name: t })))

    const byEmail = new Map<string, { name: string; email: string; team_name: string }>()
    for (const m of members) {
      const email = norm(m.email)
      if (!email) continue
      if (!byEmail.has(email)) byEmail.set(email, { name: m.name, email: m.email, team_name: m.team_name })
    }
  ...
  const filtered = useMemo(() => {
    ...
    return candidates.filter((c) => {
      ...
      if (branchFilter.length > 0 && !branchFilter.includes(c.branch)) return false
      if (expertFilter.length > 0 && !expertFilter.includes(c.expert)) return false
      if (recruiterFilter.length > 0 && !recruiterFilter.includes(c.recruiter)) return false
      ...
      if (teamMemberFilter.length > 0) {
        ...
        const ok = teamMemberFilter.some((email) => {
          ...
          return (e && (expert.includes(e) || recruiter.includes(e))) || (nm && (expert.includes(nm) || recruiter.includes(nm)))
        })
        if (!ok) return false
      }
      return true
    })
  }, [branchFilter, candidates, expertFilter, recruiterFilter, search, statusFilter, teamFilter, teamMemberFilter, teamMembers])
  ...
  const grouped = useMemo(() => {
    ...
    const teamNames = Object.keys(byTeam).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "base" }))
    return teamNames.map((team) => ({
      team,
      items: byTeam[team].slice().sort((a, b) => cmp(a, b) || a.candidateName.localeCompare(b.candidateName)),
    }))
  ...
  const exportCsv = useCallback(() => {
    ...
    const headers = ["candidate name", "branch", "expert", "recruiter", "current status", "team"]
    const lines = [
      headers.join(","),
      ...rows.map((r) =>
        [
    ...
    return (
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className={`${widthClassName} justify-between`}>
            <span className="truncate">{filterLabel(label, selected, valueToLabel)}</span>
          </Button>
        </DropdownMenuTrigger>
      ...
      <div className="space-y-6 w-full max-w-[1920px] mx-auto">
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="space-y-1">
              <h1 className="text-3xl font-bold tracking-tight text-foreground">Active Candidates</h1>
              <p className="text-muted-foreground">
                Real-time view of active candidates enriched with team assignments.
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="icon" onClick={() => fetchCandidates("refresh")} disabled={refreshing}>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="pb-3">
            ...
            <CardDescription>Filter and sort candidates by team and assignment.</CardDescription>
            <div className="flex flex-wrap items-end gap-3 mt-3">
              ...
              <MultiSelect
                ...
                values={teamMembers.map((m) => ({ value: m.email, label: `${m.name} (${m.email})` }))}
                selected={teamMemberFilter}
                onChange={setTeamMemberFilter}
                valueToLabel={(v) => teamMembers.find((m) => norm(m.email) === norm(v))?.name || v}
              />
          ...
          <CardContent className="pt-0">
            {error ? (
              <div className="p-4 rounded-md border border-border bg-background text-sm text-destructive flex items-center justify-between gap-3">
                <div>{error}</div>
                <Button variant="outline" size="sm" onClick={() => fetchCandidates("refresh")}>
                  Retry
                </Button>
              </div>
            ) : null}
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-[180px]">
                      <button type="button" className="inline-flex items-center gap-2" onClick={() => toggleSort("branch")}>
                        Branch <ArrowUpDown className="h-4 w-4" />
                      </button>
                    </TableHead>
                    <TableHead className="w-[260px]">
                      <button type="button" className="inline-flex items-center gap-2" onClick={() => toggleSort("expert")}>
                      ...
                      groupRows.push(
                        <TableRow key={`team-${g.team}`}>
                          <TableCell colSpan={5} className="bg-muted/40 font-semibold">
                            {g.team} <span className="text-muted-foreground font-normal">({g.items.length})</span>
                          </TableCell>
                        </TableRow>
                      )
                      for (const c of g.items) {
                        groupRows.push(
                          <TableRow key={c.id}>
                            ...
                            <TableCell className="text-muted-foreground">{c.expert || "-"}</TableCell>
            ...
            <div className="md:hidden mt-4 space-y-4">
              {loading ? (
                ...
                <div className="p-4 text-sm text-muted-foreground">No active candidates found.</div>
              ) : (
                grouped.map((g) => (
                  <div key={`team-cards-${g.team}`} className="space-y-3">
                    ...
                    {g.items.map((c) => (
                      <Card key={`card-${c.id}`} className="border-border bg-card">
                        <CardContent className="p-4 space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <div className="text-foreground">{c.expert || "-"}</div>
                            <div className="text-muted-foreground">Recruiter</div>
                            <div className="text-foreground">{c.recruiter || "-"}</div>
                          </div>
                        </CardContent>
                      </Card>
                    ))}
